...
function parseTSVData(tsvData) {
  ...
  const data = lines.filter(line => line && !line.startsWith('#') && !line.startsWith('5s'))
    .map(line => {
...
async function fetchWaterDataFromUSGS(siteNumber, startDate = "2023-01-01", endDate = "2024-01-26") {
